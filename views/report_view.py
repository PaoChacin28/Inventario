...
def show_generate_report_form(parent_frame, user_id):
    ...
    # --- LAYOUT DE 3 PARTES CON PACK ---

    # --- PARTE 1: SECCIÓN SUPERIOR (CONTROLES Y FILTROS) ---
    ...
    try:
        # ... (código de los widgets de filtros sin cambios) ...
    ...
    # --- PARTE 2: SECCIÓN INFERIOR (SOLO BOTONES) ---
    # --- CAMBIO: Se elimina la creación del hsb de este frame ---
    ...
    action_frame.pack(fill='x', pady=(10,0)) # Simplificado

    # --- PARTE 3: SECCIÓN CENTRAL (LA TABLA AJUSTABLE) ---
    ...
    # --- CAMBIO: Se elimina la configuración del scrollbar horizontal (xscrollcommand) ---
    ...
    def generate_report_action():
        ...
        if report_data:
            ...
            # --- CAMBIO: LÓGICA DE AJUSTE AUTOMÁTICO DE COLUMNAS ---
            # Identificamos las columnas que queremos que sean flexibles (ej. las que tienen nombres largos)
            flexible_columns = ['Producto Nombre', 'Descripcion', 'Usuario Responsable', 'Nombre']

            for key, name in report_headers.items():
                tree.heading(key, text=name)
                # Si la columna NO es flexible, le damos un ancho fijo y le decimos que NO se estire.
                if name not in flexible_columns:
                    tree.column(key, anchor='center', width=120, stretch=tk.NO)
                else:
                    # Si ES flexible, le damos un ancho mínimo pero permitimos que se estire (stretch=True por defecto).
    ...
    def export_action():
        ...
        if not report_data:
            messagebox.showwarning("Sin Datos", "Primero debe generar un reporte con datos.")
            return
        temp_path = report_controller.preview_report_as_pdf(report_data, report_headers, report_title)
        if temp_path and os.path.exists(temp_path):
            report_title_for_file = type_combobox.get().replace(" ", "_")
            report_controller.save_previewed_pdf(temp_path, report_title_for_file)
