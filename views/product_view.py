...
def show_register_product_form(parent_frame):
    """Dibuja el formulario para registrar un producto con el diseño final centrado."""
    clear_frame(parent_frame)
    ttk.Label(parent_frame, text="Registrar Nuevo Producto", style='ContentTitle.TLabel').pack(pady=20)
    
    scroll_container = ScrollableFrame(parent_frame)
    scroll_container.pack(fill="both", expand=True)
    
    # 1. Frame principal que se ancla arriba y permite el centrado
    form_frame = ttk.Frame(scroll_container.scrollable_frame, style='MainContent.TFrame')
    form_frame.pack(padx=20, pady=10, fill="x", anchor="n")
    form_frame.columnconfigure(0, weight=1) # Columna central expansiva

    # 2. Contenedor de ancho fijo que se centrará en la columna de la grilla
    ...
    entries = {}
    field_labels = {"codigo_producto": "Código Producto:* (ej. ABC-123)", "nombre": "Nombre:*", "tipo": "Tipo:*"}
    ...
    ttk.Label(fields_container, text="Paso 2: Asociar Proveedores (Opcional)", font=("Arial", 11, "bold")).pack(anchor="w", pady=(20,0))
    ttk.Separator(fields_container, orient='horizontal').pack(fill='x', pady=(2, 10))
...
def show_edit_product_form(parent_frame, product_code):
    """Dibuja el formulario para editar la definición de un producto (nombre y tipo)."""
    clear_frame(parent_frame)
    ...
    entries = {}
    # El código no se edita, solo el nombre y tipo
    field_labels = {"nombre": "Nombre", "tipo": "Tipo"}
...
def show_product_providers_details(parent_frame, product_data):
    ...
    mostrando su información de contacto.
    """
    clear_frame(parent_frame)
    product_id = product_data['id_producto']
    product_name = product_data['nombre']
    ...
    # --- CORRECCIÓN: Definición de Columnas ---
    # Quitamos "ID Proveedor" y añadimos "Teléfono" y "Dirección"
    ...
    def _load_associated_providers():
        ...
        # --- CORRECCIÓN: Inserción de Datos ---
        for p in associated_list:
            tree.insert("", "end", values=(
                p['nombre'], 
                p['rif'], 
                p.get('telefono') or 'N/A', 
                p.get('direccion') or 'N/A'
            ))
...
def show_all_products_list(parent_frame):
    """Dibuja la vista principal de productos con todas las funcionalidades."""
    clear_frame(parent_frame)
    ...
    def on_deactivate_record():
        ...
        if not selected_items:
            ...
            return
        product_code = tree.item(selected_items[0])['values'][0]
        if product_controller.handle_deactivate_product(product_code):
            _load_initial_data()
    ...
    def on_view_providers():
        selected_items = tree.selection()
        if not selected_items:
            messagebox.showwarning("Sin selección", "Por favor, seleccione un producto para ver sus proveedores.")
            return
        product_id = int(selected_items[0])
        product_data = next((p for p in all_products if p['id_producto'] == product_id), None)
        if product_data:
            show_product_providers_details(parent_frame, product_data)

    ttk.Button(action_frame, text="Añadir Producto", command=on_add_record, style='Action.TButton').pack(side='left', padx=10)
    ttk.Button(action_frame, text="Desincoporar Seleccionado", command=on_deactivate_record, style='Delete.TButton').pack(side='right', padx=10)
    ttk.Button(action_frame, text="Editar Seleccionado", command=on_edit_record, style='Action.TButton').pack(side='right', padx=(0, 5))
    ttk.Button(action_frame, text="Ver Proveedores", command=on_view_providers, style='Search.TButton').pack(side='right', padx=(0, 5))
    ttk.Button(action_frame, text="Ver Lotes", command=on_view_lots, style='Search.TButton').pack(side='right', padx=(0, 5))
